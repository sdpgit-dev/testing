...
/**
 * Class OBSPGB_Error_Collector
 *
 * A tool for collecting Obspgb-related error messages and outputting them all in one place.
 *
 * @since 1.0.0.
 */
final class OBSPGB_Error_Collector extends OBSPGB_Util_Modules implements OBSPGB_Error_CollectorInterface {
	/**
	...
	protected $dependencies = array(
		'errors'        => 'WP_Error',
		'error_display' => 'OBSPGB_Error_DisplayInterface'
	);
	...
	/**
	 * OBSPGB_Error_Collector constructor.
	 *
	 * @since 1.0.0.
	 *
	...
	public function __construct( OBSPGB_APIInterface $api = null, array $modules = array() ) {
		/**
		 * Filter: Toggle for showing Obspgb errors.
		 *
		 * @since 1.0.0.
		 *
		 * @param bool    $show_errors    True to show errors.
		 */
		$this->show_errors = apply_filters( 'obspgb_show_errors', current_user_can( 'install_themes' ) );
		...
		if ( true === $this->show_errors ) {
			$modules['error_display'] = new OBSPGB_Error_Display( $api, array( 'error' => $this ) );
		} else {
	...
	public function generate_backtrace( array $ignore_class = array(), $output = 'list' ) {
		...
		/**
		 * Filter: Change the number of steps shown in a Obspgb Error backtrace.
		 *
		 * @since 1.0.0.
		 *
		 * @param int $limit    The number of backtrace steps to show.
		 */
		$limit = absint( apply_filters( 'obspgb_error_backtrace_limit', 1 ) );
