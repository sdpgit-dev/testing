...
if ( ! class_exists( 'OBSPGB_Sections_Banner_Definition' ) ) :
/**
 * Section definition for Columns
 *
 * Class OBSPGB_Sections_Banner_Definition
 */
class OBSPGB_Sections_Banner_Definition {
	/**
	 * The one instance of OBSPGB_Sections_Banner_Definition.
	 *
	 * @var   OBSPGB_Sections_Banner_Definition
	 */
	...
	public function __construct() {
		if ( is_admin() ) {
			...
			add_action( 'obspgb_builder_banner_css', array( $this, 'style_rules' ), 10, 3 );
		}

		add_filter( 'obspgb_section_html_class', array( $this, 'html_class' ), 10, 3 );
		add_filter( 'obspgb_section_html_attrs', array( $this, 'html_attrs' ), 10, 3 );
		add_filter( 'obspgb_section_item_html_class', array( $this, 'item_html_class' ), 10, 2 );
		add_filter( 'obspgb_section_item_html_style', array( $this, 'item_html_style' ), 10, 3 );

		ttfobspgb_add_section(
			'banner',
			__( 'Banner', 'obspgb' ),
			obspgb_get_plugin_directory_uri() . 'css/builder/sections/images/banner.png',
			__( 'Display multiple types of content in a banner or a slider.', 'obspgb' ),
			array( $this, 'save' ),
			array(
	...
	public function get_settings() {
		return array(
			...
			200 => array(
				...
				'default' => ttfobspgb_get_section_default( 'title', 'banner' ),
			),
			300 => array(
				'type'    => 'text',
				'label'   => __( 'Section height (px)', 'obspgb' ),
				'name'    => 'height',
				'default' => ttfobspgb_get_section_default( 'height', 'banner' ),
			),
			400 => array(
				'type'        => 'select',
				'label'       => __( 'Responsive behavior', 'obspgb' ),
				'name'        => 'responsive',
				'default' => ttfobspgb_get_section_default( 'responsive', 'banner' ),
				'description' => __( 'Choose how the Banner will respond to varying screen widths. Default is ideal for large amounts of written content, while Aspect is better for showing your images.', 'obspgb' ),
				'options'     => ttfobspgb_get_section_choices( 'responsive', 'banner' ),
			),
			500 => array(
				'type'  => 'divider',
			...
			600 => array(
				...
				'default' => ttfobspgb_get_section_default( 'background-image', 'banner' ),
			),
			700 => array(
				'type'  => 'select',
				...
				'default' => ttfobspgb_get_section_default( 'background-position', 'banner' ),
				'options' => ttfobspgb_get_section_choices( 'background-position', 'banner' ),
			),
			800 => array(
				'type'    => 'select',
				...
				'class'   => 'ttfobspgb-configuration-media-related',
				'default' => ttfobspgb_get_section_default( 'background-style', 'banner' ),
				'options' => ttfobspgb_get_section_choices( 'background-style', 'banner' ),
			),
			900 => array(
				'type'    => 'checkbox',
				'label'   => __( 'Darken', 'obspgb' ),
				'name'    => 'darken',
				'default' => ttfobspgb_get_section_default( 'darken', 'banner' ),
			),
			1000 => array(
				'type'    => 'color',
				...
				'default' => ttfobspgb_get_section_default( 'background-color', 'banner' ),
			),
			1100 => array(
				'type'    => 'divider',
				'label'   => __( 'Slideshow', 'obspgb' ),
				'name'    => 'divider-slideshow',
				'class'   => 'ttfobspgb-configuration-divider',
			),
			1200 => array(
				'type'    => 'checkbox',
				'label'   => __( 'Navigation arrows', 'obspgb' ),
				'name'    => 'arrows',
				'default' => ttfobspgb_get_section_default( 'arrows', 'banner' ),
			),
			1300 => array(
				'type'    => 'checkbox',
				'label'   => __( 'Navigation dots', 'obspgb' ),
				'name'    => 'dots',
				'default' => ttfobspgb_get_section_default( 'dots', 'banner' ),
			),
			1400 => array(
				'type'    => 'checkbox',
				'label'   => __( 'Autoplay slideshow', 'obspgb' ),
				'name'    => 'autoplay',
				'default' => ttfobspgb_get_section_default( 'autoplay', 'banner' ),
			),
			1500 => array(
				'type'    => 'select',
				'label'   => __( 'Speed', 'obspgb' ),
				'name'    => 'delay',
				'default' => ttfobspgb_get_section_default( 'delay', 'banner' ),
				'options' => ttfobspgb_get_section_choices( 'delay', 'banner' )
			),
			1600 => array(
				'type'    => 'select',
				'label'   => __( 'Transition effect', 'obspgb' ),
				'name'    => 'transition',
				'default' => ttfobspgb_get_section_default( 'transition', 'banner' ),
				'options' => ttfobspgb_get_section_choices( 'transition', 'banner' ),
			),
		);
	}
	...
	public function get_slide_settings() {
		$inputs = array(
			100 => array(
				'type'    => 'select',
				'name'    => 'alignment',
				'label'   => __( 'Content position', 'obspgb' ),
				'default' => ttfobspgb_get_section_default( 'alignment', 'banner-slide' ),
				'options' => array(
					'none'  => __( 'None', 'obspgb' ),
					'left'  => __( 'Left', 'obspgb' ),
			...
			200 => array(
				...
				'label'   => __( 'Darken', 'obspgb' ),
				'name'    => 'darken',
				'default' => ttfobspgb_get_section_default( 'darken', 'banner-slide' )
			),
			300 => array(
				'type'    => 'color',
				'label'   => __( 'Background color', 'obspgb' ),
				'name'    => 'background-color',
				'class'   => 'ttfobspgb-gallery-background-color ttfobspgb-configuration-color-picker',
				'default' => ttfobspgb_get_section_default( 'background-color', 'banner-slide' )
			),
		);
		/**
		 ...
		 */
		$inputs = apply_filters( 'obspgb_banner_slide_configuration', $inputs );
	...
	public function section_choices( $choices, $key, $section_type ) {
		...
		switch ( $choice_id ) {
			...
			case 'banner-transition':
				$choices = array(
					'scrollHorz' => __( 'Slide horizontal', 'obspgb' ),
					'fade' => __( 'Fade', 'obspgb' ),
					'none' => __( 'None', 'obspgb' ),
				);
				break;
			...
			case 'banner-responsive' :
				$choices = array(
					'balanced' => __( 'Default', 'obspgb' ),
					'aspect'   => __( 'Aspect', 'obspgb' ),
				);
				break;
		...
		if ( $data['section-type'] == 'banner' ) {
			...
			if ( isset( $data['banner-slides'] ) && is_array( $data['banner-slides'] ) ) {
				foreach ( $data['banner-slides'] as $s => $slide ) {
					...
					$slide_image = ttfobspgb_get_image_src( $slide['background-image'], 'large' );
				...
				*/
				if( isset( $data['delay'] ) ) {
					...
					$choices = array_keys( ttfobspgb_get_section_choices( 'delay', 'banner' ) );

					if ( ! in_array( $delay, $choices ) ) {
						$data['delay'] = ttfobspgb_get_section_default( 'delay', 'banner' );
					}
				}
			}
		}
	...
	public function save( $data ) {
		...
		$clean_data['arrows'] = ( isset( $data['arrows'] ) && 1 === absint( $data['arrows'] ) ) ? 1 : 0;
		$clean_data['dots'] = ( isset( $data['dots'] ) && 1 === absint( $data['dots'] ) ) ? 1 : 0;
		$clean_data['autoplay'] = ( isset( $data['autoplay'] ) && 1 === absint( $data['autoplay'] ) ) ? 1 : 0;
		...
		if ( isset( $data['responsive'] ) ) {
			$clean_data['responsive'] = ttfobspgb_sanitize_section_choice( $data['responsive'], 'responsive', $data['section-type'] );
		}
		...
		if ( isset( $data['background-image-url'] ) && '' !== $data['background-image-url'] ) {
			...
			$clean_data['background-image-url'] = '';
		}
		...
		if ( isset( $data['banner-slides'] ) && is_array( $data['banner-slides'] ) ) {
			...
			foreach ( $data['banner-slides'] as $s => $slide ) {
				// Handle legacy data layout
				$id = isset( $slide['id'] ) ? sanitize_title_with_dashes( $slide['id'] ): $s;

				$clean_slide_data = array(
					'id' => $id,
					'section-type' => sanitize_title_with_dashes( $slide['section-type'] ),
				);
				...
				$clean_slide_data['darken'] = ( isset( $slide['darken'] ) && 1 === absint( $slide['darken'] ) ) ? 1 : 0;

				if ( isset( $slide['background-image'] ) && '' !== $slide['background-image'] ) {
					$clean_slide_data['background-image'] = ttfobspgb_sanitize_image_id( $slide['background-image'] );
				} else {
					$clean_slide_data['background-image'] = '';
				}

				if ( isset( $slide['background-image-url'] ) && '' !== $slide['background-image-url'] ) {
					$clean_slide_data['background-image-url'] = esc_url_raw( $slide['background-image-url'] );
				} else {
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_script(
			...
			obspgb_get_plugin_directory_uri() . 'js/builder/sections/banner.js',
			$dependencies,
			TTFOBSPGB_VERSION,
			true
		);
	...
	public function print_templates() {
		...
		$ttfobspgb_section_data = $section_definitions[ 'banner' ];
		?>
		<script type="text/template" id="tmpl-ttfobspgb-section-banner">
		<?php require_once( obspgb_get_plugin_directory() . '/inc/sections/banner/builder-template.php' ); ?>
		</script>
		<?php $ttfobspgb_section_data = array(); ?>
		<script type="text/template" id="tmpl-ttfobspgb-section-banner-slide">
		<?php require_once( obspgb_get_plugin_directory() . '/inc/sections/banner/builder-template-slide.php' ); ?>
		</script>
		<?php
	}
	...
	public function html_class( $classes, $section_data, $sections ) {
		if ( 'banner' === $section_data['section-type'] ) {
			 ...
			 */
			$classes = apply_filters( 'obspgb_builder_banner_class', $classes, $section_data );
		}
	...
	public function html_attrs( $attrs, $section_data ) {
		if ( 'banner' === $section_data['section-type'] ) {
			 ...
			 * @param array     $ttfobspgb_section_data    The section data.
			 */
			$attrs = apply_filters( 'obspgb_builder_get_banner_slider_atts', $attrs, $section_data );
		}
	...
	public function item_html_class( $classes, $item ) {
		if ( 'banner-slide' === $item['section-type'] ) {
			 ...
			 */
			$classes = apply_filters( 'obspgb_builder_banner_slide_class', $classes, $item );
		}
	...
	public function item_html_style( $style, $item, $section_data ) {
		if ( 'banner-slide' === $item['section-type'] ) {
			...
			// Background image
			if ( isset( $item['background-image'] ) && 0 !== ttfobspgb_sanitize_image_id( $item['background-image'] ) ) {
				$image_src = ttfobspgb_get_image_src( $item['background-image'], 'full' );
			 ...
			 * @param array     $ttfobspgb_section_data    The section data.
			 */
			$style = apply_filters( 'obspgb_builder_banner_slide_style', esc_attr( $style ), $item, $section_data );
		}
	 ...
	 * @since 1.0.0.
	 *
	 * @hooked action obspgb_builder_banner_css
	 *
	 * @param array                       $data     The banner's section data.
	 * @param int                         $id       The banner's section ID.
	 * @param OBSPGB_Style_ManagerInterface $style    The style manager instance.
	 *
	 * @return void
	 */
	public function style_rules( array $data, $id, OBSPGB_Style_ManagerInterface $style ) {
		 ...
		 */
		$section_id = apply_filters( 'obspgb_section_html_id', $prefix . $id, $data );
