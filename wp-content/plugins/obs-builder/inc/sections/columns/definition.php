...
if ( ! class_exists( 'OBSPGB_Sections_Columns_Definition' ) ) :
/**
 * Section definition for Columns
 *
 * Class OBSPGB_Sections_Columns_Definition
 */
class OBSPGB_Sections_Columns_Definition {
	/**
	 * The one instance of OBSPGB_Sections_Columns_Definition.
	 *
	 * @var   OBSPGB_Sections_Columns_Definition
	 */
	...
	public function __construct() {
		...
		ttfobspgb_add_section(
			'text', //type
			__( 'Columns', 'obspgb' ), //name
			'view_column', //icon
			__( 'Create rearrangeable content of text and images.', 'obspgb' ), //tooltip description
			array( $this, 'save' ),
			array(
	...
	public function get_settings() {
		return array(
			...
			200 => array(
				...
				'default' => ttfobspgb_get_section_default( 'title', 'text' ),
			),
            300 => array(
				'type'    => 'select',
				'name'    => 'columns-container',
				'class'   => 'ttfobspgb-text-container',
				'label'   => __( 'Container', 'obspgb' ),
				'default' => ttfobspgb_get_section_default( 'columns-container', 'text' ),
				'options' => ttfobspgb_get_section_choices( 'columns-container', 'text' ),
			),
			400 => array(
				'type'    => 'select',
				'name'    => 'columns-number',
				'class'   => 'ttfobspgb-text-columns',
				'label'   => __( 'Columns', 'obspgb' ),
				'default' => ttfobspgb_get_section_default( 'columns-number', 'text' ),
				'options' => ttfobspgb_get_section_choices( 'columns-number', 'text' ),
			),
			500 => array(
				'type'    => 'divider',
			...
			600 => array(
				...
				'default' => ttfobspgb_get_section_default( 'background-image', 'text' ),
			),
			700 => array(
				'type'  => 'select',
				...
				'default' => ttfobspgb_get_section_default( 'background-position', 'text' ),
				'options' => ttfobspgb_get_section_choices( 'background-position', 'text' ),
			),
			800 => array(
				'type'    => 'select',
				...
				'class'   => 'ttfobspgb-configuration-media-related',
				'default' => ttfobspgb_get_section_default( 'background-style', 'text' ),
				'options' => ttfobspgb_get_section_choices( 'background-style', 'text' ),
			),
			900 => array(
				'type'    => 'checkbox',
				'label'   => __( 'Darken', 'obspgb' ),
				'name'    => 'darken',
				'default' => ttfobspgb_get_section_default( 'darken', 'text' ),
			),
			1000 => array(
				'type'    => 'color',
				'label'   => __( 'Background color', 'obspgb' ),
				'name'    => 'background-color',
				'class'   => 'ttfobspgb-text-background-color ttfobspgb-configuration-color-picker',
				'default' => ttfobspgb_get_section_default( 'background-color', 'text' ),
			),
		);
	}
		...
		if ( $data['section-type'] == 'text' ) {
			...
			if ( isset( $data['columns'] ) && is_array( $data['columns'] ) ) {
				...
				foreach ( $data['columns'] as $s => $column ) {
					...
					if ( isset( $column['image-id'] ) ) {
						$column_image = ttfobspgb_get_image_src( $column['image-id'], 'large' );

						if ( isset( $column_image[0] ) ) {
							$column['image-url'] = esc_url_raw( $column_image[0] );
						}
					}
	...
	public function save( $data ) {
        ...
        if ( isset( $data['columns-container'] ) ) {
			$clean_data['columns-container'] = ttfobspgb_sanitize_section_choice( $data['columns-container'], 'columns-container', $data['section-type'] );
		}
        
		if ( isset( $data['columns-number'] ) ) {
			$clean_data['columns-number'] = ttfobspgb_sanitize_section_choice( $data['columns-number'], 'columns-number', $data['section-type'] );
		}
		...
		if ( isset( $data['columns'] ) && is_array( $data['columns'] ) ) {
			...
			foreach ( $data['columns'] as $i => $item ) {
				...
				if ( isset( $item['sidebar-label'] ) ) {
					$clean_item_data['sidebar-label'] = sanitize_title_with_dashes( $item['sidebar-label'] );
				}
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_script(
			...
			obspgb_get_plugin_directory_uri() . 'js/builder/sections/columns.js',
			$dependencies,
			TTFOBSPGB_VERSION,
			true
		);
	...
	public function print_templates() {
		...
		$ttfobspgb_section_data = $section_definitions[ 'text' ];
		?>
		<script type="text/template" id="tmpl-ttfobspgb-section-text">
		<?php require_once( obspgb_get_plugin_directory() . '/inc/sections/columns/builder-template.php' ); ?>
		</script>
		<?php $ttfobspgb_section_data = array(); ?>
		<script type="text/template" id="tmpl-ttfobspgb-section-text-item">
		<?php require_once( obspgb_get_plugin_directory() . '/inc/sections/columns/builder-template-column.php' ); ?>
		</script>
		<?php
	}
	...
	public function html_class( $classes, $section_data, $sections ) {
		if ( 'text' === $section_data['section-type'] ) {
			 ...
			 */
			$classes = apply_filters( 'obspgb_builder_get_text_class', $classes, $section_data, $sections );
		}
