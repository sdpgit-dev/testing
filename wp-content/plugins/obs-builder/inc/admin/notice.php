...
/**
 * Class OBSPGB_Admin_Notice
 *
 * Register and display notices in the Admin interface.
 *
 * @since 1.0.0.
 */
final class OBSPGB_Admin_Notice implements OBSPGB_Admin_NoticeInterface, OBSPGB_Util_HookInterface, OBSPGB_Util_LoadInterface {
	/**
	...
	public function hook() {
		...
		if ( is_admin() ) {
			...
			add_action( 'wp_ajax_obspgb_hide_notice', array( $this, 'handle_ajax' ) );
		}
	...
	public function load() {
		 ...
		 * @since 1.0.0.
		 *
		 * @param OBSPGB_Admin_Notice    $notice     The notice object that has just finished loading.
		 */
		do_action( 'obspgb_notice_loaded', $this );
	}
	...
	public function register_one_time_admin_notice( $message, WP_User $user = null, array $args = array() ) {
		...
		// Sanitize message
		$message = $args['sanitize'] ? $this->sanitize_message( $args['message'] ) : $args['message'];
	...
	private function generate_one_time_admin_notice_id( WP_User $user = null ) {
		...
		return 'obspgb-notice-' . md5( $user->user_login );
	}
	...
	private function get_dismissed_notices( $user_id ) {
		$dismissed = get_user_meta( $user_id, 'obspgb-dismissed-notices', true );

		if ( ! $dismissed ) {
			// Look for deprecated meta key
			$dismissed = get_user_meta( $user_id, 'ttfobspgb-dismissed-notices', true );

			if ( $dismissed ) {
				// Add new meta entry and delete the deprecated one
				$this->update_dismissed_notices( $user_id, $dismissed );
				delete_user_meta( $user_id, 'ttfobspgb-dismissed-notices' );
			}
		}
	...
	private function update_dismissed_notices( $user_id, array $notices ) {
		return update_user_meta( $user_id, 'obspgb-dismissed-notices', $notices );
	}
	...
	public function admin_notices() {
		// Obspgb sure files are loaded first.
		if ( ! $this->is_loaded() ) {
			$this->load();
		}
	...
	private function render_notices( $notices ) {
		...
		foreach ( $notices as $id => $args ) {
			...
			$classes = $args[ 'class' ] ? $args[ 'class' ] : array( 'notice', 'notice-' . $type );
			$nonce   = ( $dismiss && ! $onetime ) ? ' data-nonce="' . esc_attr( wp_create_nonce( 'obspgb_dismiss_' . $id ) ) . '"' : '';
			...
			// Render
			?>
			<div id="obspgb-notice-<?php echo esc_attr( $id ); ?>" class="<?php echo esc_attr( $classes ); ?>"<?php echo $nonce; ?>>
			...
			(function($) {
				$('.notice:not(.one-time)').on('click', '.notice-dismiss', function(evt) {
					...
					var $target = $(evt.target),
						...
						id      = $parent.attr('id').replace('obspgb-notice-', ''),
						nonce   = $parent.data('nonce');

					$.post(
						ajaxurl,
						{
							action : 'obspgb_hide_notice',
							nid    : id,
							nonce  : nonce
						}
					);
	 ...
	 * @since 1.0.0.
	 *
	 * @hooked action wp_ajax_obspgb_hide_notice
	 *
	 * @return void
	 */
	public function handle_ajax() {
		// Only run this during an Ajax request.
		if ( 'wp_ajax_obspgb_hide_notice' !== current_action() ) {
			return;
		}
		...
		if (
			...
			false === $nid ||
			false === $nonce ||
			! wp_verify_nonce( $nonce, 'obspgb_dismiss_' . $nid )
		) {
