 ...
 * these modules:
 *
 * - $api is a specialized instance of this class that contains all of the modules that obspgb up the theme's API. If it
 ...
 * @since 1.0.0.
 */
abstract class OBSPGB_Util_Modules implements OBSPGB_Util_ModulesInterface {
	/**
	...
	/**
	 * OBSPGB_Util_Modules constructor.
	 *
	 * @since 1.0.0.
	 *
	...
	public function __call( $name, $arguments ) {
		if ( $this->has_module( $name ) ) {
			...
			return trigger_error(
				sprintf(
					esc_html__( 'Call to undefined method %1$s::%2$s()', 'obspgb' ),
					get_class( $this ),
					esc_html( $name )
				),
				E_USER_ERROR
	...
	protected function add_module( $module_name, $module ) {
		...
		if ( ! $this->has_module( $module_name ) ) {
			...
			if ( $this->modules[ $module_name ] instanceof OBSPGB_Util_HookInterface ) {
		...
		// Module already exists. Generate a warning.
			...
			trigger_error(
				sprintf(
					esc_html__( 'The %1$s module cannot be added to %2$s because it already exists.', 'obspgb' ),
	 ...
	 * @param array                  $modules
	 */
	protected function load_dependencies( OBSPGB_APIInterface $api = null, array $modules = array() ) {
		foreach ( $this->dependencies as $dependency_name => $dependency_type ) {
			...
			if ( isset( $modules[ $dependency_name ] ) ) {
				...
				// Module is a name string. Create an instance.
					...
					if ( $class_parents && in_array( 'OBSPGB_Util_Modules', $class_parents ) ) {
			...
			trigger_error(
				sprintf(
					esc_html__( '%1$s does not have a valid %2$s dependency', 'obspgb' ),
					get_class( $this ),
					$dependency_name
				),
				E_USER_ERROR
	...
	public function get_module( $module_name ) {
		// Module exists.
		if ( $this->has_module( $module_name ) ) {
			if ( $this->modules[ $module_name ] instanceof OBSPGB_Util_LoadInterface ) {
		...
		else if ( $this->has_module( 'error' ) && $this->modules['error'] instanceof OBSPGB_Error_CollectorInterface ) {
			$this->modules['error']->add_error( 'obspgb_util_module_not_valid', sprintf( __( 'The "%1$s" module can\'t be retrieved from %2$s because it doesn\'t exist.', 'obspgb' ), esc_html( $module_name ), get_class( $this ) ) );
		}
