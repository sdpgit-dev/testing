 ...
 * @since 1.0.0.
 */
class OBSPGB_Style_CSS implements OBSPGB_Style_CSSInterface {
	/**
	...
	public function add( array $data ) {
		 ...
		 * @param array $data    The selectors and properties to add to the CSS.
		 */
		$data = apply_filters( 'obspgb_css_add', $data );
	...
	public function build() {
		...
		// Obspgb sure the 'all' array is first
	...
	private function parse_declarations( $declarations, $tab = '' ) {
		 ...
		 * routines are implemented when the developer calls `ttfobspgb_get_css->add()`. Because every property value has
		...
		foreach ( $declarations as $property => $value ) {
			 ...
			 */
			$output .= apply_filters( 'obspgb_parse_declaration', $parsed_value, $property, $value, $t, $n );
		}
		 ...
		 */
		return apply_filters( 'obspgb_css_parse_declarations', $output, $declarations, $tab );
	}
}