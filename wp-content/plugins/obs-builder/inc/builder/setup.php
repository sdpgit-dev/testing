...
/**
 * Class OBSPGB_Builder_Setup
 *
 * @since 1.0.0.
 */
class OBSPGB_Builder_Setup extends OBSPGB_Util_Modules implements OBSPGB_Builder_SetupInterface, OBSPGB_Util_HookInterface {
	/**
	...
	/**
	 * OBSPGB_Builder_Setup constructor.
	 *
	 * @since 1.0.0.
	 *
	...
	public function __construct( OBSPGB_APIInterface $api, array $modules = array() ) {
		...
		// Include the API
		require_once obspgb_get_plugin_directory() . '/inc/builder/core/api.php';

		// Load backend files
		if ( is_admin() ) {
			require_once obspgb_get_plugin_directory() . '/inc/builder/core/base.php';
		}
	}
	...
	public function hook() {
		...
		add_action( 'wp_enqueue_scripts', array( $this, 'frontend_builder_scripts' ), 20 );
		add_action( 'obspgb_style_loaded', array( $this, 'builder_styles' ) );
		add_filter( 'obspgb_get_section_data', array( $this, 'read_layout' ), 10, 2 );
	...
	/**
	 * Read the serialized section layout.
	 *
	 * @since 1.0.0.
	 *
	 * @return bool
	...
	public function read_layout( $sections, $post_id ) {
		$layout = get_post_meta( $post_id, '_ttfobspgb_layout', true );
		...
		if ( ! empty( $layout ) ) {
			...
			foreach ( $layout as $section_id ) {
				$section = get_post_meta( $post_id, "_ttfobspgb_section_{$section_id}", true );
	...
	/**
	 * Handle front end scripts for use with the existing sections on the current Builder page.
	 *
	 * @since 1.0.0.
	 *
	...
	public function frontend_builder_scripts() {
		if ( ttfobspgb_is_builder_page() ) {
			$sections = ttfobspgb_get_post_section_data( get_the_ID() );
			...
			foreach ( $section_types as $section_id => $section_type ) {
				switch ( $section_type ) {
					...
					case 'productgrid' :
						...
						if ( !defined( 'OBSPGB_LOAD_CYCLE2_DEPENDENCY' ) || OBSPGB_LOAD_CYCLE2_DEPENDENCY !== false ) {
							$this->scripts()->add_dependency( 'obspgb-frontend', 'cycle2', 'script' );
							if ( defined( 'SCRIPT_DEBUG' ) && true === SCRIPT_DEBUG ) {
								$this->scripts()->add_dependency( 'obspgb-frontend', 'cycle2-center', 'script' );
								$this->scripts()->add_dependency( 'obspgb-frontend', 'cycle2-swipe', 'script' );
							}
						}
						break;
				}
	 ...
	 * @since 1.0.0.
	 *
	 * @hooked action obspgb_style_loaded
	 *
	 * @param OBSPGB_Style_ManagerInterface $style    The style manager instance.
	 *
	 * @return void
	 */
	public function builder_styles( OBSPGB_Style_ManagerInterface $style ) {
		if ( ttfobspgb_is_builder_page() ) {
			$sections = ttfobspgb_get_post_section_data( get_the_ID() );
			...
			if ( ! empty( $sections ) ) {
				foreach ( $sections as $id => $data ) {
					if ( isset( $data['section-type'] ) ) {
						 ...
						 * @param OBSPGB_Style_ManagerInterface $style    The style manager instance.
						 */
						do_action( 'obspgb_builder_' . $data['section-type'] . '_css', $data, $id, $style );
					}
				}
			}
		}
...
if ( ! function_exists( 'ttfobspgb_get_post_section_data' ) ) :
/**
 ...
 */
function ttfobspgb_get_post_section_data( $post_id, $section_id = false ) {
	$ordered_data = array();
	$ids          = get_post_meta( $post_id, '_ttfobspgb-section-ids', true );
	...
	if ( is_array( $post_meta ) ) {
		foreach ( $post_meta as $key => $value ) {
			...
			if ( 0 === strpos( $key, '_ttfobspgb:' ) ) {
				...
				$temp_data[ str_replace( '_ttfobspgb:', '', $key ) ] = $value[0];
			}
		}
	}

	// Create multidimensional array from postmeta
	$data = ttfobspgb_create_array_from_meta_keys( $temp_data );
	...
	if ( ! isset( $GLOBALS['ttfobspgb_sections'] ) ) {
		/**
		 ...
		 */
		$ttfobspgb_sections = apply_filters( 'obspgb_get_section_data', $ordered_data, $post_id );
...
if ( ! function_exists( 'ttfobspgb_create_array_from_meta_keys' ) ) :
/**
 ...
 */
function ttfobspgb_create_array_from_meta_keys( $arr ) {
...
if ( ! function_exists( 'ttfobspgb_post_type_supports_builder' ) ) :
/**
 * Check if a post type supports the Obspgb builder.
 *
 * @since 1.0.0.
 *
 ...
 */
function ttfobspgb_post_type_supports_builder( $post_type ) {
	return post_type_supports( $post_type, 'obspgb-builder' );
}
endif;

if ( ! function_exists( 'ttfobspgb_is_builder_page' ) ) :
/**
 ...
 */
function ttfobspgb_is_builder_page( $post_id = 0 ) {
	if ( empty( $post_id ) ) {
		$post_id = get_the_ID();
	}
	...
	$has_builder_meta = get_post_meta( $post_id, '_ttfobspgb_layout', true );
	 ...
	 */
	return apply_filters( 'obspgb_is_builder_page', $is_builder_page, $post_id );
}
endif;
