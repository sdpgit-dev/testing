...
if ( ! function_exists( 'TTFOBSPGB_Builder_Save' ) ) :
/**
 * Defines the functionality for the HTML Builder.
 *
 * @since 1.0.0.
 */
class TTFOBSPGB_Builder_Save {
	/**
	 * The one instance of TTFOBSPGB_Builder_Save.
	 *
	 * @since 1.0.0.
	 *
	 * @var   TTFOBSPGB_Builder_Save
	 */
	...
	/**
	 * Instantiate or return the one TTFOBSPGB_Builder_Save instance.
	 *
	 * @since 1.0.0.
	 *
	 * @return TTFOBSPGB_Builder_Save
	 */
	...
	public function __construct() {
		...
		if ( isset( $_POST[ 'ttfobspgb-builder-nonce' ] ) && wp_verify_nonce( $_POST[ 'ttfobspgb-builder-nonce' ], 'save' ) ) {
			...
			add_action( 'obspgb_builder_data_saved', array( $this, 'save_layout' ), 10, 2 );
		}
	}
	...
	public function save_post( $post_id, $post ) {
		...
		if ( isset( $_POST['use-builder'] ) && 1 === (int) $_POST['use-builder'] ) {
			update_post_meta( $post_id, '_ttfobspgb-use-builder', 1 );
		} else {
			update_post_meta( $post_id, '_ttfobspgb-use-builder', '' );
		}

		// Don't save data if we're not using the Builder template
		if ( ! ttfobspgb_will_be_builder_page() ) {
			return;
		}
	...
	public function prepare_data( $sections ) {
		...
		foreach ( $sections as $section ) {
			$section_type = isset( $section['section-type'] ) ? sanitize_title_with_dashes( $section['section-type'] ): '';

			if ( isset( $registered_sections[ $section_type ]['save_callback'] ) && true === $this->is_save_callback_callable( $registered_sections[ $section_type ] ) ) {
				/**
				...
				$id = $section['id'];
				$clean_sections[ $id ]['id'] = sanitize_title_with_dashes( strval( $id ) );
				$clean_sections[ $id ]['state'] = ( isset( $section['state'] ) ) ? sanitize_key( $section['state'] ) : 'open';
				$clean_sections[ $id ]['section-type'] = $section_type;
				$clean_sections[ $id ] = apply_filters( 'obspgb_prepare_data_section', call_user_func_array( $registered_sections[ $section_type ]['save_callback'], array( $section ) ), $section, $section_type );
			}
		}
		 ...
		 */
		return apply_filters( 'obspgb_prepare_data', $clean_sections, $sections );
	}
	...
	public function save_data( $sections, $post_id ) {
		 ...
		 */
		$values_to_save = $this->flatten_array( $sections, '_ttfobspgb:', ':' );
		...
		update_post_meta( $post_id, '_ttfobspgb-section-ids', $section_ids );
		 ...
		 */
		do_action( 'obspgb_builder_data_saved', $sections, $post_id );
	...
	public function save_layout( $sections, $post_id ) {
		...
		delete_post_meta( $post_id, '_ttfobspgb-section-ids' );

		// Remove legacy section metas
		$post_meta = get_post_meta( $post_id );
		foreach ( $post_meta as $key => $value ) {
			if ( 0 === strpos( $key, '_ttfobspgb:' ) ) {
		...
		foreach( $sections as $id => $section ) {
			...
			update_post_meta( $post_id, '_ttfobspgb_section_' . $section[ 'id' ], $section );
			$layout[] = $section[ 'id' ];
		}

		// Purge removed sections
		$current_layout = get_post_meta( $post_id, '_ttfobspgb_layout', true );
		...
		if ( ! empty( $current_layout ) ) {
			...
			foreach ( $removed_section_ids as $section_id ) {
				delete_post_meta( $post_id, "_ttfobspgb_section_{$section_id}" );
			}
		}

		// Update layout
		update_post_meta( $post_id, '_ttfobspgb_layout', $layout );
	}
	...
	public function wp_insert_post_data( $data, $postarr ) {
		if ( ! ttfobspgb_will_be_builder_page() || ! isset( $_POST[ 'ttfobspgb-builder-nonce' ] ) || ! wp_verify_nonce( $_POST[ 'ttfobspgb-builder-nonce' ], 'save' ) ) {
			return $data;
		}
		 ...
		 */
		$sanitized_sections = apply_filters( 'obspgb_insert_post_data_sections', $this->get_sanitized_sections() );
	...
	public function generate_post_content( $data ) {
		...
		add_filter( 'obspgb_the_builder_content', array( $this, 'embed_handling' ), 8 );
		...
		// templates render through ttfobspgb_get_section_data
		global $ttfobspgb_sections, $ttfobspgb_section_data;
		$ttfobspgb_sections = $data;
		$post_id = get_the_ID();
		...
		foreach ( $ttfobspgb_sections as $section_data ) {
			$ttfobspgb_section_data = $section_data;
			$section_definition = ttfobspgb_get_section_definition( $section_data['section-type'] );
			$section_template = $section_definition['display_template'];

			if ( ttfobspgb_should_render_section( $ttfobspgb_section_data ) ) {
				ttfobspgb_get_template( $section_template );
			}
		}

		unset( $GLOBALS['ttfobspgb_sections'] );
		unset( $GLOBALS['ttfobspgb_section_data'] );
		 ...
		 */
		return apply_filters( 'obspgb_generate_post_content', $post_content, $data );
	}
	...
	public function get_sanitized_sections() {
		if ( empty( $this->_sanitized_sections ) ) {
			...
			if ( isset( $_POST['ttfobspgb-section-layout'] ) && ! empty( $_POST['ttfobspgb-section-layout'] ) ) {
				$section_ids = json_decode( wp_unslash( $_POST['ttfobspgb-section-layout'] ), true );

				if ( is_array( $section_ids ) ) {
					foreach( $section_ids as $section_id ) {
						$section_id = sanitize_title_with_dashes( $section_id );
						$section_name = 'ttfobspgb-section-json-' . $section_id;
...
if ( ! function_exists( 'ttfobspgb_get_builder_save' ) ) :
/**
 * Instantiate or return the one TTFOBSPGB_Builder_Save instance.
 *
 * @since 1.0.0.
 ...
 * @return TTFOBSPGB_Builder_Save
 */
function ttfobspgb_get_builder_save() {
	return TTFOBSPGB_Builder_Save::instance();
}
endif;

add_action( 'admin_init', 'ttfobspgb_get_builder_save' );
