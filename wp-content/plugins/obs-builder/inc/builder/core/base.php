...
if ( ! function_exists( 'TTFOBSPGB_Builder_Base' ) ) :
/**
 * Defines the functionality for the HTML Builder.
 *
 * @since 1.0.0.
 */
class TTFOBSPGB_Builder_Base {
	/**
	 * The one instance of TTFOBSPGB_Builder_Base.
	 *
	 * @since 1.0.0.
	 *
	 * @var   TTFOBSPGB_Builder_Base
	 */
	private static $instance;

	/**
	 * Instantiate or return the one TTFOBSPGB_Builder_Base instance.
	 *
	 * @since  1.0.0.
	 *
	 * @return TTFOBSPGB_Builder_Base
	 */
	...
	public function __construct() {
		...
		require_once obspgb_get_plugin_directory() . '/inc/builder/core/configuration-helpers.php';

		// Include the save routines
		require_once obspgb_get_plugin_directory() . '/inc/builder/core/save.php';
		...
		add_filter( 'obspgb_get_section_data', array( $this, 'massage_legacy_format' ), 20, 2 );
		// Disable Gutenberg
		add_filter( 'use_block_editor_for_post', '__return_false' );
	}

	/**
	 * Add support for post types to use the Obspgb builder.
	 *
	 * @since  1.3.0.
	 *
	 * @return void
	...
	public function register_post_type_support_for_builder() {
		add_post_type_support( 'page', 'obspgb-builder' );
	}
	...
	public function add_meta_boxes() {
		foreach ( ttfobspgb_get_post_types_supporting_builder() as $name ) {
			$label = 'page' === $name ?
				esc_html__( 'Page Builder', 'obspgb' ) :
				esc_html__( 'Post Builder', 'obspgb' );
	...
	public function display_builder_toggle() {
		...
		if ( ! ttfobspgb_post_type_supports_builder( get_post_type() ) ) {
			return;
		}
		 ...
		 */
		$builder_is_default = apply_filters( 'obspgb_builder_is_default', true );

		$post_id = get_the_ID();
		$builder_meta = get_post_meta( $post_id, '_ttfobspgb_layout', true );

		// Show the builder by default on new pages,
		// or if the current page holds a builder layout.
		$using_builder = ( 'post-new.php' === $pagenow && $builder_is_default ) || $builder_meta ? 1 : 0;
		$using_builder_meta_exists = metadata_exists( 'post', $post_id, '_ttfobspgb-use-builder' );
		$using_builder_meta = get_post_meta( $post_id, '_ttfobspgb-use-builder', true );
		...
		$label = 'page' === get_post_type( $post_id ) ?
				esc_html__( 'Enable page builder', 'obspgb' ) :
				esc_html__( 'Enable post builder', 'obspgb' );
	?>
		<p>
			<input type="checkbox" value="1" name="use-builder" id="use-builder"<?php checked( $using_builder, 1 ); ?> />
			&nbsp;<label for="use-builder"><?php echo $label; ?></label>
		</p>
	<?php
	}
	...
	public function display_builder( $post_local ) {
		wp_nonce_field( 'save', 'ttfobspgb-builder-nonce' );

		// Load the boilerplate templates
		require_once( obspgb_get_plugin_directory() . '/inc/builder/core/templates/menu.php' );
		require_once( obspgb_get_plugin_directory() . '/inc/builder/core/templates/stage.php' );
	}

	public function massage_legacy_format( $section_data, $post_id ) {
		$registered_sections = ttfobspgb_get_sections();

		foreach ( $section_data as $id => $section ) {
			/**
			 * In Obspgb 1.4.0, the blank section was deprecated. Any existing blank sections are converted to 1 column,
			 * text sections.
			 */
		...
		$section_order = get_post_meta( $post_id, '_ttfobspgb-section-ids', true );
	...
	public function admin_enqueue_scripts( $hook_suffix ) {
		...
		wp_enqueue_style(
			'ttfobspgb-builder',
			obspgb_get_plugin_directory_uri() . 'css/builder/core/builder.css',
			array(),
			TTFOBSPGB_VERSION
		);
		...
		/**
		 * Filter the dependencies for the Obspgb builder JS.
		 *
		 * @since 1.2.3.
		 *
		 * @param array    $dependencies    The list of dependencies.
		 */
		$dependencies = apply_filters( 'obspgb_builder_js_dependencies', $dependencies );

		wp_enqueue_script(
			'ttfobspgb-builder',
			obspgb_get_plugin_directory_uri() . 'js/builder/core/builder.js',
			$dependencies,
			TTFOBSPGB_VERSION,
			true
		);
		...
		wp_enqueue_script(
			'ttfobspgb-builder-overlay',
			obspgb_get_plugin_directory_uri() . 'js/builder/core/overlay.js',
			array( 'ttfobspgb-builder' ),
			TTFOBSPGB_VERSION,
			true
		);

		// Get the current sections
		$section_data = ttfobspgb_get_post_section_data( get_the_ID() );
		...
		wp_localize_script(
			...
			'ttfObspgbSections',
			array(
				'settings' => ttfobspgb_get_sections_settings(),
				'defaults' => ttfobspgb_get_sections_defaults(),
				'data' => ttfobspgb_get_section_json_data( $section_data )
			)
		);
		...
		$builder_settings = array(
			'pageID'        => get_the_ID(),
			'postRefresh'   => true,
			'confirmString' => esc_html__( 'Are you sure you want to trash this section permanently?', 'obspgb' ),
		);

		$builder_settings = apply_filters( 'obspgb_builder_settings', $builder_settings );

		wp_localize_script(
			'ttfobspgb-builder',
			'ttfobspgbBuilderSettings',
			$builder_settings
		);

		wp_enqueue_script(
			'ttfobspgb-builder-edit-page',
			obspgb_get_plugin_directory_uri() . 'js/builder/core/edit-page.js',
			array( 'jquery' ),
			TTFOBSPGB_VERSION,
			true
		);
		...
		$data = array(
			'featuredImage' => esc_html__( 'Note: the Builder Template does not display a featured image.', 'obspgb' ),
		...
		if ( 'page' === get_post_type( get_the_ID() ) ) {
			...
			$layout = get_post_meta( get_the_ID(), '_ttfobspgb_layout', true );
			$data['useBuilder'] = 'template-builder.php' === $template && '' !== $layout;
		}

		wp_localize_script(
			'ttfobspgb-builder-edit-page',
			'ttfobspgbEditPageData',
			$data
		);
	}
	...
	?>
		<style type="text/css">
			<?php if ( 'post-new.php' === $pagenow || ( 'post.php' === $pagenow && ttfobspgb_is_builder_page() ) ) : ?>
			#postdivrich {
				display: none;
			}
			<?php else : ?>
			...
			.ttfobspgb-duplicator {
				display: none;
			}
			<?php endif; ?>

			<?php /*foreach ( ttfobspgb_get_sections() as $key => $section ) : ?>
			#ttfobspgb-menu-list-item-link-<?php echo esc_attr( $section['id'] ); ?> .ttfobspgb-menu-list-item-link-icon-wrapper {
	...
	function admin_body_class( $classes ) {
		...
		// Do not complete the function if the product template is in use (i.e., the builder needs to be shown)
		if ( ttfobspgb_post_type_supports_builder( get_post_type() ) ) {
			if ( 'post-new.php' === $pagenow || ( 'post.php' === $pagenow && ttfobspgb_is_builder_page() ) ) {
				$classes .= ' ttfobspgb-builder-active';
			} else {
				$classes .= ' ttfobspgb-default-active';
			}
		}
	...
	public function add_uploader( $section_name, $image_id = 0, $title = '', $field_name = 'background-image-url', $field_name_path = '' ) {
		$image = ttfobspgb_get_image_src( $image_id, 'large' );
		$title = ( ! empty( $title ) ) ? $title : esc_html__( 'Set image', 'obspgb' );
		ob_start();
?>
	<div class="ttfobspgb-uploader{{ data.get('<?php echo $field_name; ?>')<?php echo $field_name_path; ?> && ' ttfobspgb-has-image-set' || '' }}">
		<div data-title="<?php echo $title; ?>" class="ttfobspgb-media-uploader-placeholder ttfobspgb-media-uploader-add" style="background-image: url({{ data.get('<?php echo $field_name; ?>')<?php echo $field_name_path; ?> }});"></div>
	</div>
	<?php
		$output = ob_get_clean();
		return $output;
	...
	public function add_frame( $id, $textarea_name, $textarea_name_path = '', $content = '', $iframe = true ) {
		global $ttfobspgb_is_js_template;
		$iframe_id   = 'ttfobspgb-iframe-' . $id;
		$textarea_id = 'ttfobspgb-content-' . $id;
		$textarea_attr_name = 'ttfobspgb-section[' .$id. '][' .$textarea_name. ']';
	?>
		<?php if ( true === $iframe ) : ?>
		<span class="ttfobspgb-iframe-content-placeholder{{ (!data.get('<?php echo $textarea_name; ?>')) ? ' show' : '' }}">
			<?php esc_html_e( 'Click to edit', 'obspgb' ); ?>
		</span>
		<div class="ttfobspgb-iframe-wrapper">
			<div class="ttfobspgb-iframe-overlay">
				<a href="#" class="edit-content-link" data-textarea="<?php echo esc_attr( $textarea_id ); ?>" data-iframe="<?php echo esc_attr( $iframe_id ); ?>">
					<span class="screen-reader-text">
						<?php esc_html_e( 'Edit content', 'obspgb' ); ?>
					</span>
				</a>
			</div>
	...
	public function print_templates() {
		...
		$ttfobspgb_overlay_id = 'ttfobspgb-tinymce-overlay';
		$ttfobspgb_overlay_class = 'ttfobspgb-overlay ttfobspgb-content-overlay';
		$ttfobspgb_overlay_title = __( 'Edit content', 'obspgb' );
		$ttfobspgb_overlay_button_label = __( 'Update changes', 'obspgb' );
		require_once( obspgb_get_plugin_directory() . '/inc/builder/core/templates/overlay-tinymce.php' );

		// Configuration overlay templates
		$ttfobspgb_overlay_id = '{{ data.id }}';
		$ttfobspgb_overlay_class = '{{ data.className }}';
		$ttfobspgb_overlay_title = '{{ data.title }}';
		$ttfobspgb_overlay_button_label = '{{ data.buttonLabel }}';
		require_once( obspgb_get_plugin_directory() . '/inc/builder/core/templates/overlay-configuration.php' );
		?>
		<script type="text/html" id="tmpl-ttfobspgb-media-overlay-remove-image">
			<div class="ttfobspgb-media-overlay-remove-image">
				<h3><?php esc_html_e( 'Current image', 'obspgb' ); ?></h3>
				<a href="#" class="ttfobspgb-remove-image-from-modal">
	 ...
	 *
	 * This is a copy and modification of the core "media_buttons" function. In order to obspgb the media editor work
	...
	public function get_section_data( $post_id ) {
		return ttfobspgb_get_post_section_data( $post_id );
	}
	...
	function create_array_from_meta_keys( $arr ) {
		return ttfobspgb_create_array_from_meta_keys( $arr );
	}
}
endif;

if ( ! function_exists( 'ttfobspgb_get_builder_base' ) ) :
/**
 * Instantiate or return the one TTFOBSPGB_Builder_Base instance.
 *
 * @since  1.0.0.
 ...
 * @return TTFOBSPGB_Builder_Base
 */
function ttfobspgb_get_builder_base() {
	return TTFOBSPGB_Builder_Base::instance();
}
endif;

// Add the base immediately
if ( is_admin() ) {
	ttfobspgb_get_builder_base();
}

if ( ! function_exists( 'ttfobspgb_get_post_types_supporting_builder' ) ) :
/**
 * Get all post types that support the Obspgb builder.
 *
 * @since  1.2.0.
 *
 * @return array    Array of all post types that support the builder.
 */
function ttfobspgb_get_post_types_supporting_builder() {
	$post_types_supporting_builder = array();

	// Inspect each post type for builder support
	foreach ( get_post_types() as $name => $data ) {
		if ( post_type_supports( $name, 'obspgb-builder' ) ) {
...
if ( ! function_exists( 'ttfobspgb_will_be_builder_page' ) ):
/**
 ...
 */
function ttfobspgb_will_be_builder_page() {
	 ...
	 */
	return apply_filters( 'obspgb_will_be_builder_page', 1 === $use_builder, '', $use_builder );
}
endif;

if ( ! function_exists( 'ttfobspgb_load_section_header' ) ) :
/**
 ...
 * @return void
 */
function ttfobspgb_load_section_header() {
	global $ttfobspgb_section_data;

	require( obspgb_get_plugin_directory() . '/inc/builder/core/templates/section-header.php' );
	 ...
	 */
	if ( isset( $ttfobspgb_section_data['id'] ) ) {
		do_action( 'obspgb_section_' . $ttfobspgb_section_data['id'] . '_before', $ttfobspgb_section_data );

		// Backcompat
		do_action( 'ttfobspgb_section_' . $ttfobspgb_section_data['id'] . '_before', $ttfobspgb_section_data );
	}
}
endif;

if ( ! function_exists( 'ttfobspgb_load_section_footer' ) ) :
/**
 ...
 * @return void
 */
function ttfobspgb_load_section_footer() {
	global $ttfobspgb_section_data;

	require( obspgb_get_plugin_directory() . '/inc/builder/core/templates/section-footer.php' );
	 ...
	 */
	if ( isset( $ttfobspgb_section_data['id'] ) ) {
		do_action( 'obspgb_section_' . $ttfobspgb_section_data['id'] . '_after', $ttfobspgb_section_data );

		// Backcompat
		do_action( 'ttfobspgb_section_' . $ttfobspgb_section_data['id'] . '_after', $ttfobspgb_section_data );
	}
}
endif;

if ( ! function_exists( 'ttfobspgb_get_wp_editor_id' ) ) :
/**
 ...
 */
function ttfobspgb_get_wp_editor_id( $data, $is_js_template ) {
	$id_base = 'ttfobspgbeditor' . $data['section']['id'];
	 ...
	 */
	return apply_filters( 'obspgb_get_wp_editor_id', $id, $data, $is_js_template );
}
endif;

if ( ! function_exists( 'ttfobspgb_get_section_name' ) ) :
/**
 ...
 */
function ttfobspgb_get_section_name( $data, $is_js_template = true ) {
	$name = 'ttfobspgb-section[{{ id }}]';
	 ...
	 * @param bool      $is_js_template    Whether or not this is in the context of a JS template.
	 */
	return apply_filters( 'obspgb_get_section_name', $name, $data, true );
}
endif;

if ( ! function_exists( 'ttfobspgb_get_section_json_data' ) ) :
	/**
	 ...
	 */
	function ttfobspgb_get_section_json_data( $data = array() ) {
		foreach ($data as $s => $section) {
			 ...
			 */
			$data[$s] = apply_filters( 'obspgb_get_section_json', $section );
		}
