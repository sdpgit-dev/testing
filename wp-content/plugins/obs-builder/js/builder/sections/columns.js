( function( $, _, Backbone, builderSettings, sectionData ) {
	...
	var View = obspgb.classes.SectionView.extend( {
		...
		onColumnCountChanged: function( itemModel ) {
			...
			var $stage = $( '.ttfobspgb-text-columns-stage', this.$el );
			...
			$stage.attr('data-layout', '');

			$stage.removeClass( function( i, className ) {
				return className.match( /ttfobspgb-text-columns-[0-9]/g || [] ).join( ' ' );
			});

			$stage.addClass( 'ttfobspgb-text-columns-' + newColumnCount );
		},
		...
		onItemViewAdded: function( itemViewModel, itemViewCollection, options ) {
			...
			if ( 0 === itemViewIndex ) {
				$( '.ttfobspgb-text-columns-stage', this.$el ).prepend( $itemViewEl );
			} else {
		...
		initSortables: function() {
			var $sortable = $( '.ttfobspgb-text-columns-stage', this.$el );

			$sortable.sortable( {
				handle: '.ttfobspgb-sortable-handle',
				placeholder: 'sortable-placeholder',
				items: '.ttfobspgb-text-column',
		...
		onAddItemClick: function( e ) {
			...
			var itemModel = obspgb.factory.model( sectionData.defaults['text-item'] );
			itemModel.parentModel = this.model;
	...
	var ItemView = obspgb.classes.SectionItemView.extend( {
		template: wp.template( 'ttfobspgb-section-text-item' ),

		events: _.extend( {}, obspgb.classes.SectionItemView.prototype.events, {
			'click .ttfobspgb-text-column-remove': 'onRemoveItemClick',
			'click .edit-content-link': 'onEditItemContentClick',
		} ),

		initialize: function() {
			obspgb.classes.SectionItemView.prototype.initialize.apply( this, arguments );
		},
		...
		afterRender: function() {
			...
			obspgb.utils.initFrame( iframe );
			obspgb.utils.setFrameContent( iframe, this.model.get( 'content' ) );
		...
		onItemContentChanged: function() {
			...
			obspgb.utils.setFrameContent( $iframe.get( 0 ), this.model.get( 'content' ) );
			this.refreshContentHeight();
		},
		...
		refreshContentHeight: function() {
			...
			if ( '' !== this.model.get( 'content' ) ) {
				$( '.ttfobspgb-iframe-content-placeholder', this.$el ).removeClass( 'show' );
				var iframeHeight = Math.min( Math.max( $iframe.contents().innerHeight(), 300 ), 500 );
				$iframe.innerHeight( iframeHeight );
			} else {
				$( '.ttfobspgb-iframe-content-placeholder', this.$el ).addClass( 'show' );
