( function ( $, _, Backbone, builderSettings, sectionData ) {
	 ...
	 *
	 */
	window.obspgb.overlays.content = Backbone.View.extend( {
		events: {
			'click .ttfobspgb-overlay-close-update': 'onUpdate',
			'click .ttfobspgb-overlay-close-discard': 'onDiscard',
			'click .ttfobspgb-overlay-wrapper': 'onWrapperClick',
		},
		...
		initialize: function( options ) {
			...
			this.$editor = $( '#wp-obspgb_content_editor-wrap' );
			this.$textarea = $( '#obspgb_content_editor' );
		...
		render: function() {
			this.setElement( document.getElementById( 'ttfobspgb-tinymce-overlay' ) );

			if ( this.buttonLabel ) {
				$( '.ttfobspgb-overlay-close-update', this.$el ).text( this.buttonLabel );
			}
		...
		getEditor: function() {
			return tinyMCE.get( 'obspgb_content_editor' );
		},
		...
		onWrapperClick: function( e ) {
			if ( $( e.target ).is( '.ttfobspgb-overlay-wrapper' ) ) {
				e.preventDefault();
				e.stopPropagation();
	 ...
	 *
	 */
	window.obspgb.overlays.media = Backbone.View.extend( {
		removeImageTemplate: wp.media.template( 'ttfobspgb-media-overlay-remove-image' ),

		events: {
			'click .ttfobspgb-remove-image-from-modal': 'onRemoveClick'
		},
		...
		initialize: function( options ) {
			...
			this.frame = wp.media.frames.frame = wp.media( {
				title: options.title,
				className: 'media-frame ttfobspgb-builder-uploader',
				multiple: false,
		...
		onFrameOpen: function() {
			...
			if ( attachmentID ) {
				...
				$( '.ttfobspgb-media-overlay-remove-image', this.$el ).show();
			}
		},
		...
		onToggleSelection: function() {
			if ( this.frame.state().get( 'selection' ).size() ) {
				...
				$( '.ttfobspgb-media-overlay-remove-image', this.$el ).hide();
			}
		},
	...
	window.obspgb.classes.configuration = {};
	 ...
	 *
	 */
	window.obspgb.overlays.configuration = Backbone.View.extend( {
		template: wp.template( 'ttfobspgb-overlay-configuration' ),
		className: 'ttfobspgb-overlay ttfobspgb-configuration-overlay',
		id: 'ttfobspgb-overlay-configuration',
		...
		render: function() {
			...
			var $body = $( '.ttfobspgb-overlay-body', this.$el );

			// Render controls
			_( this.settings ).each( function( setting ) {
				var view = window.obspgb.classes.configuration[setting.type];
			...
			$( '.ttfobspgb-configuration-divider-wrap', this.$el ).each( function() {
				$( this ).nextUntil( '.ttfobspgb-configuration-divider-wrap' ).wrapAll( '<div />' );
			} );
		...
		open: function() {
			...
			var $overlay = $( '.ttfobspgb-overlay-body', this.$el );
			var $dividers = $( '.ttfobspgb-configuration-divider-wrap', this.$el );
	 ...
	 *
	 */
	window.obspgb.classes.configuration.control = Backbone.View.extend( {
		className: 'ttfobspgb-configuration-overlay-input-wrap',
	 ...
	 *
	 */
	window.obspgb.classes.configuration.divider = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-divider' ),
		...
		settingUpdated: function(e) {
			...
			var $divider = this.$el;
			var $dividers = $( '.ttfobspgb-configuration-divider-wrap' ).not( this.$el );
			var $body = this.$el.parents( '.ttfobspgb-overlay-body' );
	 ...
	 *
	 */
	window.obspgb.classes.configuration.description = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-description' ),
	} );
	 ...
	 *
	 */
	window.obspgb.classes.configuration.section_title = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-section_title' ),
	 ...
	 *
	 */
	window.obspgb.classes.configuration.select = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-select' ),
	 ...
	 *
	 */
	window.obspgb.classes.configuration.checkbox = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-checkbox' ),
		...
		settingUpdated: function() {
			...
			window.obspgb.classes.configuration.control.prototype.settingUpdated.apply( this, arguments );
		},
	 ...
	 *
	 */
	window.obspgb.classes.configuration.text = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-text' ),
	 ...
	 *
	 */
	window.obspgb.classes.configuration.color = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-color' ),
		widget: false,

		render: function() {
			window.obspgb.classes.configuration.control.prototype.render.apply( this, arguments );
			...
			if ( builderSettings.palettes ) {
 				var palettes = _( builderSettings.palettes );
				pickerSettings.palettes = palettes.isArray() ? palettes.toArray(): palettes.values();
			}

			this.widget = $( 'input', this.$el ).wpColorPicker( pickerSettings );
		...
		getValue: function() {
			return $( '.ttfobspgb-text-background-color', this.$el ).val();
		},
	 ...
	 *
	 */
	window.obspgb.classes.configuration.image = window.obspgb.classes.configuration.control.extend( {
		template: wp.template( 'ttfobspgb-settings-image' ),

		events: {
			'click .ttfobspgb-media-uploader-placeholder': 'onMediaAdd',
		},

		initialize: function( setting, overlay ) {
			window.obspgb.classes.configuration.control.prototype.initialize.apply( this, arguments );
		...
		onBackgroundChanged: function( settingModel ) {
			...
			if ( '' !== backgroundImageURL ) {
				this.$el.addClass( 'ttfobspgb-has-image-set' );
			} else {
				this.$el.removeClass( 'ttfobspgb-has-image-set' );
			}
