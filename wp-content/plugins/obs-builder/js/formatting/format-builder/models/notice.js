...
(function(Backbone, $, _, builder) {
	 ...
	 * @since 1.4.1.
	 */
	builder.nodes.notice = 'div.ttfobspgb-notice';
	...
	builder.formats.notice = builder.FormatModel.extend({
		...
		parseAttributes: function(node) {
			...
			// Parse the icon.
			icon = $node.find('i.ttfobspgb-notice-icon');
			if ( icon.length > 0 ) {
		...
		insert: function() {
			...
			$node.find('i.ttfobspgb-notice-icon').remove();

			// Add the current icon, if one is set.
			if ('' !== this.get('icon')) {
				// Build the icon.
				$icon = $('<i>');
				$icon.attr('class', 'ttfobspgb-notice-icon fa ' + this.escape('icon') + ' pull-' + this.escape('iconPosition'));
				$icon.css({
