/* global Backbone, jQuery, _, ObspgbFormatBuilder */
...
(function(Backbone, $, _, builder) {
	 ...
	 * @since 1.4.1.
	 */
	builder.nodes.button = 'a.ttfobspgb-button';
	...
	builder.formats.button = builder.FormatModel.extend({
		...
		parseAttributes: function( node ) {
			...
			// Parse the icon.
			icon = $node.find('i.ttfobspgb-button-icon');
			if (icon.length > 0) {
			...
			classes = classes.filter( function( cssClass ) {
				return cssClass !== 'ttfobspgb-button';
			} );
		...
		insert: function() {
			...
			// Obspgb sure the right node is selected.
			...
			$node.find('i.ttfobspgb-button-icon').remove();

			// Add the current icon, if one is set.
			if ('' !== this.get('icon')) {
				// Build the icon.
				$icon = $('<i>');
				$icon.attr('class', 'ttfobspgb-button-icon fa ' + this.escape('icon'));
...
})(Backbone, jQuery, _, ObspgbFormatBuilder);