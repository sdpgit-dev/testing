 ...
 * @since 1.4.1.
 */
/* global jQuery, ObspgbDynamicStylesheet */
...
(function($, DynamicStylesheet) {
	...
	DynamicStylesheet = $.extend(DynamicStylesheet, {
		 ...
		 */
		cacheSelector: {
			$button: 'a.ttfobspgb-button[data-hover-color], a.ttfobspgb-button[data-hover-background-color]',
			$list: 'ul.ttfobspgb-list[data-icon-color]'
		},
		...
		createStylesheet: function() {
			...
			this.stylesheet = this.stylesheet || (function() {
				...
				// Add an id
				$style.attr('id', 'ttfobspgb-dynamic-styles');
		...
		removeStylesheet: function() {
			if (this.root.find('head').length > 0) {
				$('#ttfobspgb-dynamic-styles', this.root).remove();
			} else {
				this.root.parent().find('#ttfobspgb-dynamic-styles').remove();
			}
...
})(jQuery, ObspgbDynamicStylesheet);