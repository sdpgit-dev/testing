...
if ( ! function_exists( 'wpdemo_setup' ) ) :
	...
	function wpdemo_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'wpdemo' ),
		) );
 ...
 */
function wpdemo_scripts() {
	$subsetsOS = 'latin,latin-ext';
    $protocol = is_ssl() ? 'https' : 'http';
	$query_argsOS = array(
        'family' => 'Roboto:300,400,500,700,900',
        'subset' => $subsetsOS,
    );
	wp_enqueue_style( 'roboto-fonts', add_query_arg( $query_argsOS, "$protocol://fonts.googleapis.com/css" ), array(), null );
	wp_enqueue_style( 'wpdemo-style', get_stylesheet_uri() );

	wp_enqueue_script( 'wpdemo-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'wpdemo-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
	
	wp_enqueue_script( 'wpdemo-main', get_template_directory_uri() . '/js/main.js', array(), '20151215', true );
	
	if ( is_singular() && comments_open() && get_option( 'thread_comments' ) ) {
		wp_enqueue_script( 'comment-reply' );
	}
}
add_action( 'wp_enqueue_scripts', 'wpdemo_scripts' );
